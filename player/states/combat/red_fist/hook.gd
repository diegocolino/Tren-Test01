## Hook (W3). Fases internas: anticipation → release → recovery.
class_name Hook extends State
...
func enter(_prev: StringName, _msg: Dictionary = {}) -> void:
	...
	kive.current_hit_type = "hook"
	kive.last_w_executed = "hook"
	...
	match phase:
		...
		"recovery":
			...
			if in_cancel_window and Input.is_action_just_pressed("attack_kick"):
				if DebugOverlay.show_debug_text:
					print("[Hook] CANCEL TIGHT → FrontalKick at recovery_t=%.3f" % phase_timer)
				return &"FrontalKick"
